...
public class RouletteStrategy implements ImpressionStrategy {
    ...
    /**
     * Строит карту весов баннеров, где "площадь", занимаемая баннером прямо
     * пропорциональная его весу, случайно выбирает точку на этой карте и
     * возвращает баннер, к оторому эта точка привязана. Таким образом, чем выше
     * вес баннера, тем ыше вероятность его показа.
     *
    ...
    public Banner chooseBanner(Request request) {
        ...
        /**
         * TODO: отфильтровать баннера (уже просмотренные, не подходящие по
         * ограничениям)
         */
        if (banners.isEmpty()) {
            return null;
        ...
        Integer key = 0;
        /**
         * генерим случайное число и смотрим, в отрезок какого баннера оно
         * попало
         */
